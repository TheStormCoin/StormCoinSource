...
uint256 hashGenesisBlock("0x523a49ba8de47624a71eb6ddfd6b451f1ac7d68252f194cb8624a735e7deb98c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // StormCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "StormCoin Signed Message:\n";
...
{
    // StormCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 86400;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // StormCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // StormCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // StormCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x523a49ba8de47624a71eb6ddfd6b451f1ac7d68252f194cb8624a735e7deb98c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1393509128;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084718137;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // StormCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("StormCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("StormCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static StormCoinMiner(CWallet *pwallet)
{
    printf("StormCoinMiner:\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("StormCoin-miner");
    ...
    try { loop {
        ...
        printf("Running StormCoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("StormCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&StormCoinMiner, pwallet));
}
